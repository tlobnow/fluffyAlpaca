...
cat slurm* > ~/fluffyAlpaca/output_files/${2}/slurm.out
mkdir -p ${LOC_SCRIPTS}/${1}/temp/
mv slurm* ${LOC_SCRIPTS}/${1}/temp/

# ENTER THE OUTPUT FOLDER
cd ~/fluffyAlpaca/output_files/$2

# RENAME FILES (PKL FILES ARE MOVED TO A JOINED PKL FOLDER)
for i in {1..5}; do
  mv model_${i}_*_*_*_*.pdb ${2}_model_${i}.pdb
  [ -f model_${i}_*_*_*_*.pkl ]; rm model_${i}_*_*_*_*.pkl
  mv relaxed_model_${i}_*   ${2}_rlx_model_${i}.pdb
  mv ranking_model_${i}_*   ${2}_ranking_model_${i}.json
done
